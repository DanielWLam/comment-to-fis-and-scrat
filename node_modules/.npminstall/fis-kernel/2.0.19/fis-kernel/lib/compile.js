...
var exports = module.exports = function(file){
    ...
    file = fis.file.wrap(file); //封装成File对象
    if(!file.realpath){
...
function pipe(file, type, ext, keep){
    var key = type + ext; //eg: preprocessor.js
    fis.util.pipe(key, function(processor, settings, key){ //processor: preckey: preprocessor.components
