...
//eg: pipe('preprocessor.js', cb, undefined)
_.pipe = function(type, callback, def){ //preprocessor.js, cb, undefined
    var processors = fis.config.get('modules.' + type, def); //'components'
    if(processors){
        var typeOf = typeof processors;
        if(typeOf === 'string'){
            processors = processors.trim().split(/\s*,\s*/); //['components']
        } else if(typeOf === 'function'){
            processors = [ processors ];
        }
        type = type.split('.')[0]; //preprocessor
        processors.forEach(function(processor, index){
            var typeOf = typeof processor, key; //processor: 'components'
            if(typeOf === 'string'){
                key = type + '.' + processor; //preprocessor.components
                processor = fis.require(type, processor); //(preprocessor, components) ===> require('fis-preprocessor-components')
            } else {
                key = type + '.' + index;
            }
